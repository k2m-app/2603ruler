...
def sort_histories(histories, target_course, target_distance):
    def score_hist(h):
        c_match = (h['course'] == target_course)
        d_match = (str(h['distance']) == str(target_distance))
...
def format_direct_chain(base_horse, current_runners, race_info, is_banei, umaban_dict):
    ...
    horses_in_race = []
    base_time = race_info['horses'].get(base_horse)
    if base_time is None: return ""
    
    for h in current_runners:
        if h in race_info['horses']:
    ...
    for u in current_runners:
        for v in current_runners:
            ...
            try:
                ...
                # 直接対決時は、最優先の履歴データ（sort_histories）を絶対視する
                if len(path) == 2 and (G.has_edge(u, v) or G.has_edge(v, u)):
                    edge = G[u][v] if G.has_edge(u, v) else G[v][u]
                    best_hist = sort_histories(edge['history'], target_course, target_distance)[0]
                    score = best_hist['raw_diff'] if G.has_edge(u, v) else -best_hist['raw_diff']
        ...
        for comp_idx, component in enumerate(valid_components):
            ...
            for tier in display_order:
                if tier == "!":
                    if comp_idx == 0 and unranked_horses:
                        ...
                        for h in unranked_horses:
                            output.append(f"<div style='margin-bottom: 10px; font-weight: bold; color: #2c3e50;'>{format_horse_name(h, umaban_dict)}</div>")
                    continue
                ...
                for horse, diff in horses_in_tier:
                    ...
                    horse_disp = f"{star_prefix}{format_horse_name(horse, umaban_dict)}"
                    ...
                    for opp in list(component):
                        ...
                        if G.has_edge(horse, opp) or G.has_edge(opp, horse):
                            ...
                            best_hist = sort_histories(edge['history'], target_course, target_distance)[0]
                            direct_race_ids.add(best_hist['race_id'])
                            
                    if direct_race_ids:
                        direct_html.append("<div style='font-size:0.85em; font-weight:bold; color:#2980b9; margin-top:5px;'>🔍直接対決</div>")
                        race_infos = [next((r for r in past_races if r['race_id'] == rid), None) for rid in direct_race_ids]
                        race_infos = [r for r in race_infos if r is not None]
                        race_infos.sort(key=lambda x: x['date'], reverse=True) # 日付の新しい順
                        
                        for rinfo in race_infos:
                            link = f"<a href='https://db.netkeiba.com/race/{rinfo['race_id']}' target='_blank' style='color:#3498db; text-decoration:none;'>{rinfo['date']} {rinfo['course'][0:1]}{rinfo['distance']}</a>"
                    ...
                    for opp in list(component):
                        ...
                        if not (G.has_edge(horse, opp) or G.has_edge(opp, horse)) and runner_G.has_edge(horse, opp):
                            try:
                                ...
                                hidden_names = ",".join([f"{h}" for h in hidden_nodes])
                                ...
                                if diff_val >= 0:
                                    ...
                                    hidden_html.append(f"<div style='font-size:0.85em; margin-left:8px;'>[{hidden_names}] {opp_str}(-{abs_diff:.1f}){sep}本馬</div>")
                            except:
                                pass
...
def rank_horses(race_id, mark_race_id=None):
    ...
    result_html_content, track_scores, track_unranked, track_fastest, track_tier_map = analyze_all_horses_html(G_unified, past_races, umaban_dict, target_course, target_distance, html_race_id, is_banei=is_banei)
